!function() {
    function a(a, b) {
        ...
        b = b || "&",
        c = [];
        for (d in a) c.push(encodeURIComponent(d) + "=" + encodeURIComponent(a[d]));
        return c.join(b)
    }
    function b(b, c) {
        var d, e, f = new Image,
        ...
        if (g && h) {
            for (g += h, e = d = 0; e < g.length; e++) d += g.charCodeAt(e);
            c._ = 100 * h + d % 100
        }
    ...
    function c(a, b, c, e) {
        ...
        c && (f = new Date, f.setTime(f.getTime() + 864e5 * c), g = "; expires=" + f.toGMTString()),
        c && e && d(a),
        a = a + "=" + b + g + "; path=/",
        e && (a += ";domain=" + e),
        document.cookie = a
    ...
    function d(a) {
        ...
        for (a = 0; a < c.length; a++) document.cookie = b + "domain=" + c[a],
    ...
    function g(a, b) {
        ...
        r = {};
        for (c = 0; c < o.length; c++) d = o[c],
        (g = e(d)) && (r[d] = g);
        if (h = document.referrer, h && location.hostname == l(h) && (h = f(h, location.hostname)), b ? r.e = (new Date).getTime() - m: window.performance && window.performance.timing && (i = window.performance.timing, i.fetchStart && (r.e = m - i.fetchStart)), r.r = h, r._ = Math.round((new Date).getTime() / 1e3), j = "undefined" != typeof _PK ? _PK: document.getElementById("page_key_name") && document.getElementById("page_key_name").getAttribute("value"), j && (r.pk = j), a) for (k in a) r[k] = a[k];
        return r
    }
    function h() {
        ...
        if (!r) {
            ...
            g) for (0 === g.indexOf("%22") && (g = g.substr(3, g.length - 6)), g = g.split("&"), i = 0; i < g.length; i++) y = g[i].split("="),
            ...
            d || (c(o, g), d = g + "");
            a: {
                for (y = window.location.search.substring(1).split("&"), i = 0; i < y.length; i++) if (h = y[i].split("="), 2 == h.length && "sid" == decodeURIComponent(h[0])) {
            ...
            c(p, a(z, "&")),
            b && 8 == b.split(".").length ? (x = b.split("."), j = x[0], k = x[1], l = x[2], m = x[3], s = +x[4], u = +x[5], v = +x[6], w = +x[7], s += 1, m != d ? (l = m, m = d, u += 1, v = 1) : v += 1) : (j = Math.floor(1e8 * Math.random()), k = g, l = "", m = d, s = 1, u = 1, v = 1, w = 0),
            i && (w = 0),
    ...
    function k(a, c, d, e) {
        if (!a) throw "event track's label params is required";
        h(),
        a = {
            ca: a
        },
    ...
    v = !1,
    w = "";
    0 <= u.indexOf("bosszhipin") || 0 <= u.indexOf("weizhipin") || 0 <= u.indexOf(".zhipin") || 0 <= u.indexOf(".yushanapp") || 0 <= u.indexOf(".zdzhushou") || 0 <= u.indexOf(".wordhr") ? (v = !0, w = t ? "t.bosszhipin.com": "t.weizhipin.com") : w = t ? "t.kanzhun.com": "192.168.1.251",
    l = function() {
    ...
    function() {
        ...
        if (c.length) {
            for (a = [], b = 0; b < c.length;) a.push(c[b].getAttribute("value")),
            b += 1;
    ...
    window.onerror = function(a, b, c, d, e) {
        return "Script error." == a || b ? (setTimeout(function() {
            var a, f, g, h;
            if (d = d || window.event && window.event.errorCharacter || 0, h = d, e && e.stack) a = e.stack.toString();
            else if (arguments.callee) {
                for (a = [], f = arguments.callee.caller, g = 3; f && 0 < --g && (a.push(f.toString()), f !== f.caller);) f = f.caller;
                a = a.join(",")
            }
            kk("errorjs", b + ":" + c + ":" + h, a)
        },
                ...
                if (k) {
                    ...
                    if (p && p.indexOf("#") !== 0 && p.indexOf("javascript:;") === -1 && !target.getAttribute("noa")) {
                        ...
                        if (flag) {
                            ...
                            } else if(p.indexOf("tel:") == -1 && p.indexOf("mailto:") == -1) {//鎺掗櫎tel鍜宮ailto
